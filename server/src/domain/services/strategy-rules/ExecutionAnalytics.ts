...
export class ExecutionAnalytics {
  ...
  getStats(periodMs?: number): ExecutionStats {
    ...
    for (const [exchange, metrics] of exchangeGroups) {
      ...
      byExchange.set(exchange, {
        ...
        fillRate:
          metrics.length > 0
            ? (exchangeSuccessful.length / metrics.length) * 100
            : 0,
        avgSlippageBps: this.average(
          exchangeSuccessful.map((m) => m.slippageBps),
        ),
        avgFillTimeMs: this.average(
          exchangeSuccessful.map((m) => m.fillTimeMs),
        ),
      });
    }
  ...
  estimateSlippage(
    ...
    const slippageBps =
      Math.abs((avgPrice - depth.midPrice) / depth.midPrice) * 10000;
  ...
  isRetryableError(
    ...
    return config.retryableErrors.some((retryable) =>
      errorType.toUpperCase().includes(retryable),
    );
  }
  ...
  getRecommendedPositionSize(
    ...
    const availableLiquidity = Math.min(
      depth.depth1Pct.bidSize,
      depth.depth1Pct.askSize,
    );
