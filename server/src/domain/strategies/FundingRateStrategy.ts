...
export class FundingRateStrategy implements IExecutableStrategy {
  ...
  async execute(context: MarketDataContext): Promise<StrategyExecutionResult> {
    ...
    try {
      ...
      const position =
        onChainPosition.size > 0 ? onChainPosition : this.localPosition;
  ...
  private async handleNoPosition(
    ...
    const effectiveEquity =
      equity > 0
        ? equity
        : this.config.maxPositionSize / this.config.targetLeverage;
    const positionSizeUSD = Math.min(
      effectiveEquity * this.config.targetLeverage,
  ...
  private async handleExistingPosition(
    ...
    if (entryPrice > 0) {
      ...
      if (isAdverseMove) {
        ...
        const txHash = await this.executor.closePosition(
          this.config.contractAddress,
        );
    ...
    const flipThreshold =
      this.config.minFundingRateThreshold * this.FLIP_THRESHOLD_MULTIPLIER;

    if (absRate >= flipThreshold) {
      // Strong reversal - flip position
      this.logger.log(
        `[${this.name}] Flipping position: Rate reversed to ${(currentRate * 100).toFixed(4)}%`,
      );
      ...
      const markPrice = await this.executor.getMarkPrice(this.config.asset);
      const positionSizeUSD = Math.min(
        equity * this.config.targetLeverage,
        this.config.maxPositionSize,
      );
      const positionSizeUnits = positionSizeUSD / markPrice;
      ...
      this.logger.log(
        `[${this.name}] Closing position: Rate unfavorable at ${(currentRate * 100).toFixed(4)}%`,
      );
  ...
  async getMetrics(): Promise<Record<string, number | string>> {
    try {
      ...
      const estimatedAPY =
        Math.abs(currentRate) * this.FUNDING_PERIODS_PER_DAY * 365 * 100;
