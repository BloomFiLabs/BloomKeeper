...
export class NAVReporter implements OnModuleInit {
  ...
  private readonly navHistory: Array<{ nav: bigint; timestamp: Date; pnl: bigint }> = [];
  ...
  constructor(
    ...
    this.reportIntervalMs = this.configService.get<number>('NAV_REPORT_INTERVAL_MS', 3600000);
  }

  async onModuleInit() {
    if (!this.strategyAddress) {
      this.logger.warn('KEEPER_STRATEGY_ADDRESS not configured, NAV reporter disabled');
      return;
    }
  ...
  async calculateNAV(): Promise<NAVCalculation> {
    ...
    try {
      ...
      this.logger.warn(`Failed to get keeper Arbitrum balance: ${error.message}`);
    }
  ...
  async reportNAV(): Promise<boolean> {
    ...
    try {
      ...
      this.logger.log(`NAV reported successfully in block ${receipt.blockNumber}`);
  ...
  getTimeUntilNextReport(): number {
    ...
    const nextReportTime = this.lastReportTimestamp.getTime() + this.reportIntervalMs;
