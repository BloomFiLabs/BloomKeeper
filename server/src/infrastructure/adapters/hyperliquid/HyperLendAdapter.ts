...
export class HyperLendAdapter {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!rpcUrl) {
      this.logger.warn('HYPERLIQUID_RPC_URL not set - HyperLend adapter will not function');
      return;
    }
    ...
    if (privateKey) {
      ...
      this.logger.log(`HyperLend Adapter initialized for wallet: ${this.wallet.address}`);
    }
    ...
    if (this.config.lendingPoolAddress) {
      ...
      this.logger.warn('⚠️ HyperLend contract addresses not configured. Please set HYPERLEND_POOL_ADDRESS');
    }
  }
  ...
  private initializeContracts() {
    ...
    this.usdc = new ethers.Contract(this.config.usdcAddress, ERC20_ABI, this.wallet);
    this.weth = new ethers.Contract(this.config.wethAddress, ERC20_ABI, this.wallet);
  }
  ...
  async depositCollateral(amountUSD: number): Promise<string> {
    ...
    // Approve USDC spend
    const approveTx = await this.usdc.approve(this.config.lendingPoolAddress, amount);
  ...
  async repayETH(amountETH: number): Promise<string> {
    ...
    // Approve WETH spend
    const approveTx = await this.weth.approve(this.config.lendingPoolAddress, amount);
    await approveTx.wait();
  ...
  async getETHBorrowRate(): Promise<number> {
    ...
    const reserveData = await this.lendingPool.getReserveData(this.config.wethAddress);
