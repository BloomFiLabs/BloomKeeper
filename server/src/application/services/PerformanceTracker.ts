...
export class PerformanceTracker {
  ...
  async trackPerformance(
    ...
    const totalFeesCollected = harvests.reduce(
    ...
    const hoursSinceStart =
      (now.getTime() - this.deploymentTime.getTime()) / (1000 * 60 * 60);
    const daysSinceStart = hoursSinceStart / 24;

    // Calculate APY
    const dailyAPY =
      daysSinceStart > 0 ? (netProfit / initial / daysSinceStart) * 100 : 0;
    const annualizedAPY = dailyAPY * 365;

    // Fees per day
    const feesPerDay =
      daysSinceStart > 0 ? totalFeesCollected / daysSinceStart : 0;

    // Average rebalance cost
    const averageRebalanceCost =
      costs.length > 0 ? totalGasCosts / costs.length : 0;

    // Fees per harvest
    const feesPerHarvest =
      harvests.length > 0 ? totalFeesCollected / harvests.length : 0;

    // Last harvest info
    const lastHarvestAmount =
      harvests.length > 0 ? harvests[harvests.length - 1] : undefined;
    const lastHarvestTime =
      harvestTimesList.length > 0
        ? harvestTimesList[harvestTimesList.length - 1]
        : undefined;
  ...
  logPerformance(strategyName: string, metrics: PerformanceMetrics) {
    ...
    this.logger.log(
      `💰 Initial Deposit:        $${metrics.totalDeposited.toFixed(2)}`,
    );
    this.logger.log(
      `📈 Current NAV:            $${metrics.currentNAV.toFixed(2)}`,
    );
    this.logger.log(
      `✨ Total Fees Earned:      $${metrics.totalFeesEarned.toFixed(4)}`,
    );
    this.logger.log(
      `💵 Fees Collected:         $${metrics.totalFeesCollected.toFixed(4)} (${metrics.harvestCount} harvests)`,
    );
    this.logger.log(
      `⛽ Total Gas Costs:        $${metrics.totalGasCosts.toFixed(4)}`,
    );
    this.logger.log(
      `💵 Net Profit:             $${metrics.netProfit.toFixed(4)} (${metrics.roi.toFixed(2)}% ROI)`,
    );
    this.logger.log(
      '───────────────────────────────────────────────────────────',
    );
    this.logger.log(
      `📊 Daily APY:              ${metrics.dailyAPY.toFixed(2)}%`,
    );
    this.logger.log(
      `📊 Annualized APY:         ${metrics.annualizedAPY.toFixed(2)}%`,
    );
    this.logger.log(
      `📅 Fees Per Day:           $${metrics.feesPerDay.toFixed(4)}`,
    );
    this.logger.log(
      `💰 Avg Per Harvest:        $${metrics.feesPerHarvest.toFixed(4)}`,
    );
    if (metrics.lastHarvestTime && metrics.lastHarvestAmount !== undefined) {
      const minutesAgo = Math.floor(
        (Date.now() - metrics.lastHarvestTime.getTime()) / 60000,
      );
      this.logger.log(
        `🕐 Last Harvest:           $${metrics.lastHarvestAmount.toFixed(4)} (${minutesAgo} min ago)`,
      );
    }
    this.logger.log(
      '───────────────────────────────────────────────────────────',
    ...
    this.logger.log(
      `⚡ Avg Rebalance Cost:     $${metrics.averageRebalanceCost.toFixed(4)}`,
    );
    this.logger.log(
      `⏱️  Time Running:           ${metrics.timeSinceStart.toFixed(2)} hours`,
    );
    this.logger.log(
      '═══════════════════════════════════════════════════════════',
    );
  ...
  recordRebalance(strategyAddress: string, gasCostUSD: number) {
    ...
    this.logger.log(
      `🔄 Rebalance #${costs.length} completed | Gas: $${gasCostUSD.toFixed(4)}`,
    );
  }
  ...
  recordHarvest(strategyAddress: string, feesCollectedUSD: number) {
    ...
    this.logger.log(
      `💰 Harvest #${harvests.length} completed | Fees: $${feesCollectedUSD.toFixed(4)} | Total: $${total.toFixed(4)}`,
    );
  }
